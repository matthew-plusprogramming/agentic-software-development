...
- Always include: `agents/workflows/oneoff.workflow.md`, `agents/workflows/oneoff-spec.workflow.md`, `agents/memory-bank/project.brief.md`, `agents/memory-bank/operating-model.md`, `agents/memory-bank/task-spec.guide.md`, and the current task spec or workstream spec (review the Decision & Work Log for approvals).
...
- After each phase (Requirements, Design, Implementation Planning, Execution), log a reflection in the active task spec and record approvals in the Decision & Work Log; when stable, roll up into an ADR or a relevant stable memory bank file.
