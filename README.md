...
- After each phase, append a 3-line reflection to `agents/ephemeral/active.context.md`.
...
- `node agents/scripts/append-memory-entry.mjs`: Append reflections to `agents/ephemeral/active.context.md` (reset with `node agents/scripts/reset-active-context.mjs` when you need a fresh template).
...
3. Fill in Memory Bank canonicals (project brief, product/tech context) and keep `agents/ephemeral/active.context.md` flowing during work.
